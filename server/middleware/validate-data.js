module.exports = (req, res, next) => {
    ...
    if(!nameRegex.exec(req.body.firstName) || !nameRegex.exec(req.body.lastName)) {
        isError = true;
        errorBody["Invalid Name"] = "Entered first name or last name is invalid";

    }
    if(req.body.email) {
        if(!emailRegex.exec(req.body.email)) {
            isError = true;
            errorBody["Invalid Email"] = "Entered email is invalid";    
        }
    }
    if(req.body.phoneNumber) {
        if(!phoneNumberRegex.exec(req.body.phoneNumber)) {
            isError = true;
            errorBody["Invalid Phone Number"] = "Entered phone number is invalid";    
        }
    }
    if(isError) {
